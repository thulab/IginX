...
public class ParseTest {
    ...
    private Statement buildOperator(String sql) {
    ...
    public void testParseInsert() {
        ...
        InsertStatement op = (InsertStatement) buildOperator(insertStr);
    ...
    public void testParseFloatAndInteger() {
        ...
        InsertStatement op = (InsertStatement) buildOperator(floatAndIntegerStr);
    ...
    public void testParseSelect() {
        ...
        SelectStatement op = (SelectStatement) buildOperator(selectStr);

        assertTrue(op.isHasFunc());
        assertTrue(op.isHasValueFilter());
        assertTrue(op.isHasGroupBy());
        assertEquals(SelectStatement.QueryType.MixedQuery, op.getQueryType());

        assertEquals(4, op.getSelectedFuncsAndPaths().size());
        assertEquals(SelectStatement.FuncType.Max.toString().toLowerCase(), op.getSelectedFuncsAndPaths().get(0).k.toLowerCase());
    ...
    public void testParseSpecialClause() {
        ...
        op = (SelectStatement) buildOperator(orderByAndLimit);
    ...
    public void testAggregateAndOrderBy() {
        ...
        SelectStatement op = (SelectStatement) buildOperator(aggregateAndOrderBy);
    }

    @Test
    public void testParseDelete() {
        String deleteStr = "DELETE FROM a.b.c, a.b.d WHERE time in [1627464728862, 2022-12-12 16:18:23+1s);";
        DeleteStatement op = (DeleteStatement) buildOperator(deleteStr);
    ...
    public void testParseDeleteTimeSeries() {
        ...
        DeleteTimeSeriesStatement op = (DeleteTimeSeriesStatement) buildOperator(deleteTimeSeriesStr);
        List<String> paths = Arrays.asList("a.b.c", "a.b.d");
        assertEquals(paths, op.getPaths());
    }
    ...
    public void testTimeRange() {
        ...
        // [10, 15] -> [10, 16)
        SelectStatement op = (SelectStatement) buildOperator(lsrs);
        assertEquals(10, op.getStartTime());
        assertEquals(16, op.getEndTime());

        // (10, 15) -> [11, 15)
        op = (SelectStatement) buildOperator(lrrr);
        assertEquals(11, op.getStartTime());
        assertEquals(15, op.getEndTime());

        // [10, 15) -> [10, 15)
        op = (SelectStatement) buildOperator(lsrr);
        assertEquals(10, op.getStartTime());
        assertEquals(15, op.getEndTime());

        // (10, 15] -> [11, 16)
        op = (SelectStatement) buildOperator(lrrs);
        assertEquals(11, op.getStartTime());
        assertEquals(16, op.getEndTime());
    }
    ...
    public void testParseLimitClause() {
        ...
        SelectStatement op = (SelectStatement) buildOperator(selectWithLimit);
        assertEquals(10, op.getLimit());
        assertEquals(0, op.getOffset());

        op = (SelectStatement) buildOperator(selectWithLimitAndOffset01);
        assertEquals(10, op.getLimit());
        assertEquals(2, op.getOffset());

        op = (SelectStatement) buildOperator(selectWithLimitAndOffset02);
        assertEquals(10, op.getLimit());
        assertEquals(2, op.getOffset());

        op = (SelectStatement) buildOperator(selectWithLimitAndOffset03);
        assertEquals(10, op.getLimit());
        assertEquals(2, op.getOffset());
    }
    ...
    public void testParseShowReplication() {
        ...
        ShowReplicationStatement op = (ShowReplicationStatement) buildOperator(showReplicationStr);
        assertEquals(Statement.StatementType.SHOW_REPLICATION, op.statementType);
    }
    ...
    public void testParseAddStorageEngine() {
        ...
        AddStorageEngineStatement op = (AddStorageEngineStatement) buildOperator(addStorageEngineStr);
