...
public class NaiveFragmentGenerator implements IFragmentGeneratorV2 {

    private IMetaManager iMetaManager;
    private NaiveSampler naiveSampler;

    public NaiveFragmentGenerator(IMetaManager iMetaManager) {
        this.iMetaManager = iMetaManager;
        this.naiveSampler = NaiveSampler.getInstance();
    }

    public NaiveSampler getNaiveSampler() {
        return naiveSampler;
    }
...
//                storageUnit.addReplica(new StorageUnitMeta(RandomStringUtils.randomAlphanumeric(16), getStorageEngineList().get((i * 2 + 1) % getStorageEngineList().size()).getId(), masterId, false));
    ...
    public Pair<List<FragmentMeta>, List<StorageUnitMeta>> generateFragmentsAndStorageUnits(long startTime) {
        List<String> prefixList = naiveSampler.samplePrefix(iMetaManager.getStorageEngineList().size() - 1);
        return generateFragmentsAndStorageUnits(prefixList, startTime);
    }
