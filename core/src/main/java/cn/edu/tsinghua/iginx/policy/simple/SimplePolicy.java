...
//        }
    ...
    public Pair<List<FragmentMeta>, List<StorageUnitMeta>> generateInitialFragmentsAndStorageUnits(DataStatement statement) {
        ...
        if (ConfigDescriptor.getInstance().getConfig().getFragments().length()>0){
            Pair<Map<TimeSeriesInterval, List<FragmentMeta>>, List<StorageUnitMeta>> pair = generateInitialFragmentsAndStorageUnitsByFragments(paths, timeInterval);
     ...
     */
    public Pair<Map<TimeSeriesInterval, List<FragmentMeta>>, List<StorageUnitMeta>> generateInitialFragmentsAndStorageUnitsByFragments(List<String> paths, TimeInterval timeInterval) {
