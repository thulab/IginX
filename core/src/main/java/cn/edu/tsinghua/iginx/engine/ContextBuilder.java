...
public class ContextBuilder {
  ...
  public RequestContext build(InsertColumnRecordsReq req) {
    return buildFromInsertReq(req.getSessionId(), RawDataType.Column, req.getPaths(),
        req.getDataTypeList(),
        req.getTimestamps(), req.getValuesList(), req.getBitmapList());
  }

  public RequestContext build(InsertNonAlignedColumnRecordsReq req) {
    return buildFromInsertReq(req.getSessionId(), RawDataType.NonAlignedColumn, req.getPaths(),
        req.getDataTypeList(),
        req.getTimestamps(), req.getValuesList(), req.getBitmapList());
  }

  public RequestContext build(InsertRowRecordsReq req) {
    return buildFromInsertReq(req.getSessionId(), RawDataType.Row, req.getPaths(),
        req.getDataTypeList(),
        req.getTimestamps(), req.getValuesList(), req.getBitmapList());
  }

  public RequestContext build(InsertNonAlignedRowRecordsReq req) {
    return buildFromInsertReq(req.getSessionId(), RawDataType.NonAlignedRow, req.getPaths(),
        req.getDataTypeList(),
        req.getTimestamps(), req.getValuesList(), req.getBitmapList());
  }

  private RequestContext buildFromInsertReq(long sessionId, RawDataType rawDataType,
      List<String> paths, List<DataType> types,
    ...
    if (rawDataType == RawDataType.Row || rawDataType == RawDataType.NonAlignedRow) {
      bitmaps = bitmapList.stream().map(x -> new Bitmap(paths.size(), x.array()))
          .collect(Collectors.toList());
      values = ByteUtils.getRowValuesByDataType(valueList, types, bitmapList);
    } else {
      bitmaps = bitmapList.stream().map(x -> new Bitmap(times.size(), x.array()))
          .collect(Collectors.toList());
  ...
  public RequestContext build(DeleteDataInColumnsReq req) {
    DeleteStatement statement = new DeleteStatement(req.getPaths(), req.getStartTime(),
        req.getEndTime());
    return new RequestContext(req.getSessionId(), statement);
  }
