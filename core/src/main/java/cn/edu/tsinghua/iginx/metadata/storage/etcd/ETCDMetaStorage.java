...
public class ETCDMetaStorage implements IMetaStorage {
  ...
  public ETCDMetaStorage() {
    ...
    this.schemaMappingWatcher = client.getWatchClient()
        .watch(ByteSequence.from(SCHEMA_MAPPING_PREFIX.getBytes()),
            WatchOption.newBuilder().withPrefix(ByteSequence.from(SCHEMA_MAPPING_PREFIX.getBytes()))
                  ...
                  switch (event.getEventType()) {
                    case PUT:
                      schemaMapping = JsonUtils.getGson()
                          .fromJson(new String(event.getKeyValue().getValue().getBytes()),
    ...
    this.iginxWatcher = client.getWatchClient().watch(ByteSequence.from(IGINX_PREFIX.getBytes()),
        WatchOption.newBuilder().withPrefix(ByteSequence.from(IGINX_PREFIX.getBytes()))
            .withPrevKV(true).build(),
        new Watch.Listener() {
          @Override
          public void onNext(WatchResponse watchResponse) {
            ...
            for (WatchEvent event : watchResponse.getEvents()) {
              ...
              switch (event.getEventType()) {
                case PUT:
                  iginx = JsonUtils
                      .fromJson(event.getKeyValue().getValue().getBytes(), IginxMeta.class);
                  logger.info(
                      "new iginx comes to cluster: id = " + iginx.getId() + " ,ip = " + iginx
                          .getIp() + " , port = " + iginx.getPort());
                  ETCDMetaStorage.this.iginxChangeHook.onChange(iginx.getId(), iginx);
                  break;
                case DELETE:
                  iginx = JsonUtils
                      .fromJson(event.getPrevKV().getValue().getBytes(), IginxMeta.class);
                  logger.info(
                      "iginx leave from cluster: id = " + iginx.getId() + " ,ip = " + iginx.getIp()
                          + " , port = " + iginx.getPort());
    ...
    // 注册 storage 的监听
    this.storageWatcher = client.getWatchClient()
        .watch(ByteSequence.from(STORAGE_PREFIX.getBytes()),
            WatchOption.newBuilder().withPrefix(ByteSequence.from(STORAGE_PREFIX.getBytes()))
                .withPrevKV(true).build(),
            new Watch.Listener() {
              @Override
              public void onNext(WatchResponse watchResponse) {
                ...
                for (WatchEvent event : watchResponse.getEvents()) {
                  ...
                  switch (event.getEventType()) {
                    case PUT:
                      storageEngine = JsonUtils.fromJson(event.getKeyValue().getValue().getBytes(),
                          StorageEngineMeta.class);
                      storageChangeHook.onChange(storageEngine.getId(), storageEngine);
                      break;
                    case DELETE:
                      storageEngine = JsonUtils.fromJson(event.getPrevKV().getValue().getBytes(),
                          StorageEngineMeta.class);
    ...
    // 注册 storage unit 的监听
    this.storageUnitWatcher = client.getWatchClient()
        .watch(ByteSequence.from(STORAGE_UNIT_PREFIX.getBytes()),
            WatchOption.newBuilder().withPrefix(ByteSequence.from(STORAGE_UNIT_PREFIX.getBytes()))
                .withPrevKV(true).build(),
            new Watch.Listener() {
              @Override
              public void onNext(WatchResponse watchResponse) {
                ...
                for (WatchEvent event : watchResponse.getEvents()) {
                  ...
                  switch (event.getEventType()) {
                    case PUT:
                      storageUnit = JsonUtils.fromJson(event.getKeyValue().getValue().getBytes(),
    ...
    // 注册 fragment 的监听
    this.fragmentWatcher = client.getWatchClient()
        .watch(ByteSequence.from(FRAGMENT_PREFIX.getBytes()),
            WatchOption.newBuilder().withPrefix(ByteSequence.from(FRAGMENT_PREFIX.getBytes()))
                .withPrevKV(true).build(),
            new Watch.Listener() {
              @Override
              public void onNext(WatchResponse watchResponse) {
                ...
                for (WatchEvent event : watchResponse.getEvents()) {
                  ...
                  switch (event.getEventType()) {
                    case PUT:
                      fragment = JsonUtils
                          .fromJson(event.getKeyValue().getValue().getBytes(), FragmentMeta.class);
    ...
    this.userWatcher = client.getWatchClient().watch(ByteSequence.from(USER_PREFIX.getBytes()),
        WatchOption.newBuilder().withPrefix(ByteSequence.from(USER_PREFIX.getBytes()))
            .withPrevKV(true).build(),
        new Watch.Listener() {
          @Override
          public void onNext(WatchResponse watchResponse) {
            ...
            for (WatchEvent event : watchResponse.getEvents()) {
              ...
              switch (event.getEventType()) {
                case PUT:
                  userMeta = JsonUtils
                      .fromJson(event.getKeyValue().getValue().getBytes(), UserMeta.class);
                  userChangeHook.onChange(userMeta.getUsername(), userMeta);
                  break;
                case DELETE:
                  userMeta = JsonUtils
                      .fromJson(event.getPrevKV().getValue().getBytes(), UserMeta.class);
    ...
    try {
      ...
      GetResponse response = this.client.getKVClient()
          .get(ByteSequence.from(SCHEMA_MAPPING_PREFIX.getBytes()),
              GetOption.newBuilder().withPrefix(ByteSequence.from(SCHEMA_MAPPING_PREFIX.getBytes()))
                  .build())
          .get();
      response.getKvs().forEach(e -> {
        String schema = e.getKey().toString(StandardCharsets.UTF_8)
            .substring(SCHEMA_MAPPING_PREFIX.length());
        Map<String, Integer> schemaMapping = JsonUtils.getGson()
            .fromJson(e.getValue().toString(StandardCharsets.UTF_8),
                new TypeToken<Map<String, Integer>>() {
                }.getType());
    ...
    try {
      if (schemaMapping == null) {
        this.client.getKVClient()
            .delete(ByteSequence.from((SCHEMA_MAPPING_PREFIX + schema).getBytes())).get();
      } else {
        this.client.getKVClient()
            .put(ByteSequence.from((SCHEMA_MAPPING_PREFIX + schema).getBytes()),
                ByteSequence.from(JsonUtils.toJson(schemaMapping))).get();
      }
    } catch (ExecutionException | InterruptedException e) {
  ...
  public long registerIginx(IginxMeta iginx) throws MetaStorageException {
    try {
      ...
      this.client.getKVClient()
          .put(ByteSequence.from((IGINX_PREFIX + String.format("%07d", id)).getBytes()),
    ...
    try {
      ...
      GetResponse response = this.client.getKVClient()
          .get(ByteSequence.from(STORAGE_PREFIX.getBytes()),
              GetOption.newBuilder().withPrefix(ByteSequence.from(STORAGE_PREFIX.getBytes()))
                  .build())
          .get();
      if (response.getCount() != 0L) { // 服务器上已经有了，本地的不作数
        response.getKvs().forEach(e -> {
          StorageEngineMeta storageEngine = JsonUtils
              .fromJson(e.getValue().getBytes(), StorageEngineMeta.class);
    ...
    try {
      ...
      GetResponse response = this.client.getKVClient()
          .get(ByteSequence.from(STORAGE_UNIT_PREFIX.getBytes()),
              GetOption.newBuilder().withPrefix(ByteSequence.from(STORAGE_UNIT_PREFIX.getBytes()))
                  .build())
          .get();
      List<KeyValue> kvs = response.getKvs();
      kvs.sort(Comparator.comparing(e -> e.getKey().toString(StandardCharsets.UTF_8)));
      for (KeyValue kv : kvs) {
        StorageUnitMeta storageUnit = JsonUtils
            .fromJson(kv.getValue().getBytes(), StorageUnitMeta.class);
        if (!storageUnit.isMaster()) { // 需要加入到主节点的子节点列表中
          ...
          if (masterStorageUnit == null) { // 子节点先于主节点加入系统中，不应该发生，报错
            logger.error("unexpected storage unit " + new String(kv.getValue().getBytes())
  ...
  public void updateStorageUnit(StorageUnitMeta storageUnitMeta) throws MetaStorageException {
    try {
      client.getKVClient()
          .put(ByteSequence.from((STORAGE_UNIT_PREFIX + storageUnitMeta.getId()).getBytes()),
    ...
    try {
      ...
      GetResponse response = this.client.getKVClient()
          .get(ByteSequence.from(FRAGMENT_PREFIX.getBytes()),
              GetOption.newBuilder().withPrefix(ByteSequence.from(FRAGMENT_PREFIX.getBytes()))
                  .build())
          .get();
  ...
  public void updateFragment(FragmentMeta fragmentMeta) throws MetaStorageException {
    try {
      client.getKVClient().put(ByteSequence.from(
          (FRAGMENT_PREFIX + fragmentMeta.getTsInterval().toString() + "/" + fragmentMeta
              .getTimeInterval().toString()).getBytes()),
  ...
  public List<UserMeta> loadUser(UserMeta userMeta) throws MetaStorageException {
    try {
      ...
      GetResponse response = this.client.getKVClient()
          .get(ByteSequence.from(USER_PREFIX.getBytes()),
  ...
  public void updateUser(UserMeta userMeta) throws MetaStorageException {
    try {
      lockUser();
      this.client.getKVClient()
          .put(ByteSequence.from((USER_PREFIX + userMeta.getUsername()).getBytes()),
              ByteSequence.from(JsonUtils.toJson(userMeta))).get();
    } catch (ExecutionException | InterruptedException e) {
