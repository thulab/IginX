...
public final class ColumnDataView extends DataView {
  ...
  public ColumnDataView(RawData data, int startPathIndex, int endPathIndex, int startTimeIndex,
      int endTimeIndex) {
  ...
  public BitmapView getBitmapView(int index) { // 对于列数据来说，第一个维度为序列，所以要 checkPath
    ...
    return new BitmapView(data.getBitmaps().get(startPathIndex + index), startTimeIndex,
        endTimeIndex);
  }
}
