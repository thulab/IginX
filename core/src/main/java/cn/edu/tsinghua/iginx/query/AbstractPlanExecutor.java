...
public abstract class AbstractPlanExecutor implements IPlanExecutor, IService, IStorageEngine {
    ...
    protected AbstractPlanExecutor() {
        ...
        asyncTaskExecuteThreadPool = new ThreadPoolExecutor(ConfigDescriptor.getInstance().getConfig().getAsyncExecuteThreadPool(),
                ConfigDescriptor.getInstance().getConfig().getAsyncExecuteThreadPool(),
                0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<>(ConfigDescriptor.getInstance().getConfig().getAsyncQueueSize()));
