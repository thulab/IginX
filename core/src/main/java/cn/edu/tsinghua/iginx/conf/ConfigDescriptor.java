...
public class ConfigDescriptor {
  ...
  private void loadPropsFromFile() {
    try (InputStream in = new FileInputStream(
        EnvUtils.loadEnv(Constants.CONF, Constants.CONFIG_FILE))) {
      ...
      config.setDatabaseClassNames(properties.getProperty("databaseClassNames",
          "iotdb=cn.edu.tsinghua.iginx.iotdb.IoTDBPlanExecutor,influxdb=cn.edu.tsinghua.iginx.influxdb.InfluxDBPlanExecutor"));
      ...
      config.setPolicyClassName(properties
          .getProperty("policyClassName", "cn.edu.tsinghua.iginx.policy.naive.NativePolicy"));
      ...
      config.setMqttPayloadFormatter(properties.getProperty("mqtt_payload_formatter",
  ...
  private void loadPropsFromEnv() {
    ...
    config
        .setStorageEngineList(EnvUtils.loadEnv("storageEngineList", config.getStorageEngineList()));
    config.setMaxAsyncRetryTimes(
    ...
    config
        .setEnableRestService(EnvUtils.loadEnv("enableRestService", config.isEnableRestService()));
    ...
    config
        .setConstraintChecker(EnvUtils.loadEnv("constraintChecker", config.getConstraintChecker()));
    config
        .setPhysicalOptimizer(EnvUtils.loadEnv("physicalOptimizer", config.getPhysicalOptimizer()));
    config.setMemoryTaskThreadPoolSize(
        EnvUtils.loadEnv("memoryTaskThreadPoolSize", config.getMemoryTaskThreadPoolSize()));
    config.setPhysicalTaskThreadPoolSizePerStorage(EnvUtils
        .loadEnv("physicalTaskThreadPoolSizePerStorage",
            config.getPhysicalTaskThreadPoolSizePerStorage()));
    config.setMaxCachedPhysicalTaskPerStorage(EnvUtils
        .loadEnv("maxCachedPhysicalTaskPerStorage", config.getMaxCachedPhysicalTaskPerStorage()));
    config.setCachedTimeseriesProb(
    ...
    config
        .setFragmentPerEngine(EnvUtils.loadEnv("fragmentPerEngine", config.getFragmentPerEngine()));
