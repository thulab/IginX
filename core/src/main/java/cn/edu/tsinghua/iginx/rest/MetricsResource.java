...
public class MetricsResource {
  ...
  private static final ExecutorService threadPool = Executors
      .newFixedThreadPool(config.getAsyncRestThreadPool());
  ...
  public void addAnnotation(@Context HttpHeaders httpheaders, final InputStream stream,
  ...
  public Response postQuery(String jsonStr, boolean isAnnotation, boolean isGrafana) {
    try {
      ...
      QueryParser parser = new QueryParser();
      Query query = isAnnotation ? parser.parseAnnotationQueryMetric(jsonStr, isGrafana)
          : parser.parseQueryMetric(jsonStr);
      QueryExecutor executor = new QueryExecutor(query);
      QueryResult result = executor.execute(false);
      String entity = isAnnotation ? parser.parseResultToAnnotationJson(result, isGrafana)
          : parser.parseResultToJson(result, false);
      return setHeaders(Response.status(Status.OK).entity(entity + "\n")).build();
