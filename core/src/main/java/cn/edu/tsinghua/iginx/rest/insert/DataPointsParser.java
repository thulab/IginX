...
public class DataPointsParser {
  ...
  private void sendMetricsData() throws Exception {
    for (Metric metric : metricList) {
      ...
      valuesList[0] = values;
      try {
        session.insertNonAlignedColumnRecords(paths,
            metric.getTimestamps().stream().mapToLong(Long::longValue).toArray(), valuesList, type,
            null);
        if (metric.getAnnotation() != null) {
          ...
          session.insertNonAlignedColumnRecords(paths,
              metric.getTimestamps().stream().mapToLong(Long::longValue).toArray(), valuesList,
              type, null);
        }
