...
public class PhysicalStatisticsCollector extends AbstractStageStatisticsCollector implements
  ...
  protected void processStatistics(Statistics statistics) {
    ...
    Pair<Long, Long> detailInfo = detailInfos
        .computeIfAbsent(statistics.getContext().getStatement().getType(), e -> new Pair<>(0L, 0L));
    detailInfo.k += 1;
  ...
  public void broadcastStatistics() {
    ...
    for (Map.Entry<StatementType, Pair<Long, Long>> entry : detailInfos.entrySet()) {
      logger.info(
          "\t\tFor Request: " + entry.getKey() + ", count: " + entry.getValue().k + ", span: "
              + entry.getValue().v + "μs");
    }
