...
public class InsertGenerator extends AbstractGenerator {
  ...
  protected Operator generateRoot(Statement statement) {
    ...
    List<String> pathList = SortUtils
        .mergeAndSortPaths(new ArrayList<>(insertStatement.getPaths()));

    TimeSeriesInterval interval = new TimeSeriesInterval(pathList.get(0),
        pathList.get(pathList.size() - 1));

    Map<TimeSeriesInterval, List<FragmentMeta>> fragments = metaManager
        .getFragmentMapByTimeSeriesInterval(interval);
    if (fragments.isEmpty()) {
      //on startup
      policy.setNeedReAllocate(false);
      Pair<List<FragmentMeta>, List<StorageUnitMeta>> fragmentsAndStorageUnits = policy
          .generateInitialFragmentsAndStorageUnits(insertStatement);
      metaManager.createInitialFragmentsAndStorageUnits(fragmentsAndStorageUnits.v,
          fragmentsAndStorageUnits.k);
      ...
      Pair<List<FragmentMeta>, List<StorageUnitMeta>> fragmentsAndStorageUnits = policy
          .generateFragmentsAndStorageUnits(insertStatement);
      metaManager
  ...
  private DataView getDataSection(FragmentMeta meta, RawData rawData) {
    ...
    while (endTimeIndex < insertTimes.size() && timeInterval.getEndTime() > insertTimes
        .get(endTimeIndex)) {
      endTimeIndex++;
    }
    ...
    if (rawData.isRowData()) {
      ...
      return new ColumnDataView(rawData, startPathIndex, endPathIndex, startTimeIndex,
          endTimeIndex);
    }
  }
}
