...
public class DeleteGenerator extends AbstractGenerator {
  ...
  protected Operator generateRoot(Statement statement) {
    ...
    List<String> pathList = SortUtils
        .mergeAndSortPaths(new ArrayList<>(deleteStatement.getPaths()));
    ...
    if (fragments.isEmpty()) {
      ...
      Pair<List<FragmentMeta>, List<StorageUnitMeta>> fragmentsAndStorageUnits = policy
          .generateInitialFragmentsAndStorageUnits(deleteStatement);
    ...
    fragments.forEach((k, v) -> v.forEach(fragmentMeta -> {
      ...
      if (deleteStatement.isDeleteAll()) {
        ...
        List<TimeRange> overlapTimeRange = getOverlapTimeRange(timeInterval,
