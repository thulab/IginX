...
public class IoTDBStorage implements IStorage {
  ...
  private SessionPool createSessionPool() {
    ...
    String username = extraParams.getOrDefault(USERNAME, DEFAULT_USERNAME);
    String password = extraParams.getOrDefault(PASSWORD, DEFAULT_PASSWORD);
    int sessionPoolSize = Integer
        .parseInt(extraParams.getOrDefault(SESSION_POOL_SIZE, DEFAULT_SESSION_POOL_SIZE));
  ...
  public TaskExecuteResult execute(StoragePhysicalTask task) {
    ...
    if (operators.size() < 1) {
      return new TaskExecuteResult(new NonExecutablePhysicalTaskException(
  ...
  private TaskExecuteResult executeQueryTask(String storageUnit, Project project,
    ...
    try {
      ...
      String statement = String
          .format(QUERY_DATA, builder.deleteCharAt(builder.length() - 1).toString(), storageUnit,
  ...
  private TaskExecuteResult executeInsertTask(String storageUnit, Insert insert) {
    ...
    if (e != null) {
      return new TaskExecuteResult(null,
          new PhysicalException("execute insert task in iotdb11 failure", e));
    }
        ...
        for (int j = 0; j < data.getPathNum(); j++) {
          ...
          if (bitmapView.get(j)) {
            ...
            if (data.getDataType(j) == BINARY) {
              tablet.addValue(measurement, deviceIdToRow.get(deviceId),
  ...
  private Exception insertNonAlignedRowRecords(RowDataView data, String storageUnit) {
    ...
    for (int i = 0; i < data.getPathNum(); i++) {
      ...
      tablets.put(deviceId, new Tablet(deviceId, Collections
          .singletonList(new MeasurementSchema(measurement, toIoTDB(data.getDataType(i)))),
          batchSize));
        ...
        for (int i = cnt; i < cnt + size; i++) {
          ...
          if (bitmapView.get(i)) {
            ...
            for (Integer j : entry.getValue()) {
              ...
              if (data.getDataType(j) == BINARY) {
                tablet
                    .addValue(measurement, row, new Binary((byte[]) data.getValue(j, indexes[j])));
              } else {
  ...
  private Exception insertNonAlignedColumnRecords(ColumnDataView data, String storageUnit) {
    ...
    for (int i = 0; i < data.getPathNum(); i++) {
      ...
      measurementNum = deviceIdToCnt.computeIfAbsent(deviceId, x -> -1);
      deviceIdToCnt.put(deviceId, measurementNum + 1);
      pathIndexes = tabletIndexToPathIndexes
          .computeIfAbsent(measurementNum + 1, x -> new ArrayList<>());
      pathIndexes.add(i);
          ...
          for (int j = cnt; j < cnt + size; j++) {
            ...
            if (bitmapView.get(j)) {
              ...
              if (data.getDataType(index) == BINARY) {
                tablet.addValue(measurement, row,
  ...
  private TaskExecuteResult executeDeleteTask(String storageUnit, Delete delete) {
    if (delete.getTimeRanges() == null || delete.getTimeRanges().size() == 0) { // 没有传任何 time range
      ...
      if (paths.size() == 1 && paths.get(0).equals("*")) {
        try {
          sessionPool
              .executeNonQueryStatement(String.format(DELETE_STORAGE_GROUP_CLAUSE, storageUnit));
        } catch (IoTDBConnectionException | StatementExecutionException e) {
          logger.warn("encounter error when clear data: " + e.getMessage());
          if (!e.getMessage().contains(DOES_NOT_EXISTED)) {
            return new TaskExecuteResult(
                new PhysicalTaskExecuteFailureException("execute clear data in iotdb11 failure",
                    e));
          }
        }
      } else {
        for (String path : paths) {
          try {
            sessionPool.executeNonQueryStatement(
                String.format(DELETE_TIMESERIES_CLAUSE, storageUnit + "." + path));
          } catch (IoTDBConnectionException | StatementExecutionException e) {
            logger.warn("encounter error when delete path: " + e.getMessage());
            if (!e.getMessage().contains(DOES_NOT_EXISTED)) {
              return new TaskExecuteResult(new PhysicalTaskExecuteFailureException(
      ...
      try {
        List<String> paths = delete.getPatterns().stream().map(x -> PREFIX + storageUnit + "." + x)
            .collect(Collectors.toList());
        for (TimeRange timeRange : delete.getTimeRanges()) {
          sessionPool
              .deleteData(paths, timeRange.getActualBeginTime(), timeRange.getActualEndTime());
        }
      } catch (IoTDBConnectionException | StatementExecutionException e) {
        logger.warn("encounter error when delete data: " + e.getMessage());
        if (!e.getMessage().contains(DOES_NOT_EXISTED)) {
          return new TaskExecuteResult(
              new PhysicalTaskExecuteFailureException("execute delete data task in iotdb11 failure",
                  e));
        }
      }
    }
