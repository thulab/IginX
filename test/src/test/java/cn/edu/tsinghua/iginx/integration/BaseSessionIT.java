...
public abstract class BaseSessionIT {
  ...
  private void insertNumRecords(List<String> insertPaths)
  ...
  public void sessionTest() throws ExecutionException, SessionException, InterruptedException {
    ...
    // aggrMax
    SessionAggregateQueryDataSet maxDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.MAX);
...
//        assertEquals(simpleLen, maxDataSet.getTimestamps().length);
    ...
    // aggrMin
    SessionAggregateQueryDataSet minDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.MIN);
...
//        assertEquals(simpleLen, minDataSet.getTimestamps().length);
    ...
    //aggrFirst
    SessionAggregateQueryDataSet firstDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.FIRST_VALUE);
...
//        assertEquals(simpleLen, firstDataSet.getTimestamps().length);
    ...
    //aggrLast
    SessionAggregateQueryDataSet lastDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.LAST_VALUE);
...
//        assertEquals(simpleLen, lastDataSet.getTimestamps().length);
    ...
    //aggrCount
    SessionAggregateQueryDataSet countDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.COUNT);
    ...
    //aggrSum
    SessionAggregateQueryDataSet sumDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.SUM);
    ...
    //aggrAvg
    SessionAggregateQueryDataSet avgDataSet = session
        .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    if (isAbleToDelete) {
      ...
      SessionAggregateQueryDataSet delPartAvgDataSet = session
          .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      SessionAggregateQueryDataSet delPartCountDataSet = session
          .aggregateQuery(paths, START_TIME, END_TIME + 1, AggregateType.COUNT);
      ...
      SessionQueryDataSet delDsAvgDataSet = session
          .downsampleQuery(paths, START_TIME, END_TIME + 1, AggregateType.AVG, PRECISION);
...
//                            assertNull(dsResult.get(j));
              ...
              assertEquals((dsStartTime + delStartTime - 1) / 2.0 + pathNum,
                  changeResultToDouble(dsResult.get(j)), delta);
            } else {
              assertEquals((dsEndTime + (delEndTime - 1) + 1) / 2.0 + pathNum,
                  changeResultToDouble(dsResult.get(j)), delta);
            }
          } else {
            assertEquals(delDsAvg + pathNum, changeResultToDouble(dsResult.get(j)), delta);
      ...
      Thread.sleep(1000);
      SessionQueryDataSet delDataInColumnDataSet = session
          .queryData(delDataInColumnPaths, START_TIME, END_TIME + 1);
      ...
      SessionAggregateQueryDataSet delDataAvgSet = session
          .aggregateQuery(delDataInColumnPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
...
//                    assertEquals("null", new String((byte[]) delDataAvgResult[i]));
      ...
      SessionQueryDataSet dsDelDataInColSet = session
          .downsampleQuery(delDataInColumnPaths, START_TIME, END_TIME + 1, AggregateType.AVG,
              PRECISION);
...
//                        assertNull(dsResult.get(j));
      ...
      SessionQueryDataSet delAllColumnDataSet = session
          .queryData(delAllColumnPaths, START_TIME, END_TIME + 1);
      ...
      SessionQueryDataSet delPartColumnDataSet = session
          .queryData(partColumnPaths, START_TIME, END_TIME + 1);
      ...
      for (int j = 0; j < 6; j++) {
        ...
        switch (currPathPos) {
          ...
          case 3:
            ...
            break;
          case 4:
            assertEquals(((END_TIME - i) + 4 + 0.01) * 999, changeResultToDouble(result.get(j)),
                delta);
            break;
          case 5:
    ...
    //max
    SessionAggregateQueryDataSet dtMaxDataSet = session
        .aggregateQuery(dTAggrPaths, START_TIME, END_TIME + 1, AggregateType.MAX);
    ...
    //avg
    SessionAggregateQueryDataSet dtAvgDataSet = session
        .aggregateQuery(dTAggrPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      switch (currPathPos) {
        case 1:
          assertEquals((START_TIME + END_TIME) / 2.0 + 1, changeResultToDouble(dtAvgResult[i]),
              delta);
          break;
        case 2:
          ...
          break;
        case 3:
          assertEquals((START_TIME + END_TIME) / 2.0 + 3 + 0.01,
              changeResultToDouble(dtAvgResult[i]), delta * 10000);
          break;
        case 4:
          assertEquals((START_TIME + END_TIME) * 999 / 2.0 + 4.01 * 999,
              changeResultToDouble(dtAvgResult[i]), delta * 10000);
          break;
        default:
          fail();
    ...
    if (isAbleToDelete) {
      ...
      Thread.sleep(1000);
      SessionQueryDataSet dtDelPartDataSet = session
          .queryData(dataTypePaths, START_TIME, END_TIME + 1);
        ...
        for (int j = 0; j < 6; j++) {
          ...
          switch (currPathPos) {
            ...
            case 2:
              ...
              break;
            case 4:
              assertEquals((4 + (END_TIME - timestamp) + START_TIME + 0.01) * 999,
                  changeResultToDouble(result.get(j)), delta);
              break;
            case 1:
              if (delStartTime <= timestamp && timestamp < delEndTime) {
                assertNull(result.get(j));
      ...
      // Test aggregate function for the delete
      SessionAggregateQueryDataSet dtDelPartAvgDataSet = session
          .aggregateQuery(dTAggrPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
        ...
        switch (currPathPos) {
          case 1:
            assertEquals(((START_TIME + END_TIME) * TIME_PERIOD / 2.0 - (END_TIME -
                    (dtDelStartTime - START_TIME) + END_TIME - (dtDelEndTime - 1 - START_TIME))
                    * dtDelTimePeriod / 2.0)
                    / (TIME_PERIOD - dtDelTimePeriod) + 1.0,
                changeResultToDouble(dtDelPartAvgResult[i]), delta * 10000);
            break;
          case 2:
          ...
          case 3:
            assertEquals(((START_TIME + END_TIME) * TIME_PERIOD / 2.0 -
                    (dtDelStartTime + dtDelEndTime - 1) * dtDelTimePeriod / 2.0) / (TIME_PERIOD
                    - dtDelTimePeriod) + 3.01, changeResultToDouble(dtDelPartAvgResult[i]),
                delta * 10000);
            break;
          case 4:
            assertEquals((START_TIME + END_TIME) * 999 / 2.0 + 4.01 * 999,
      ...
      Thread.sleep(1000);
      SessionQueryDataSet dtDelColDataSet = session
          .queryData(dataTypePaths2, START_TIME, END_TIME + 1);
          ...
          if (currPathPos < dtDelColumnNum) {
            ...
            switch (getPathNum(dtDelColResPaths.get(j)) - currPath) {
              case 0:
                ...
                break;
              case 1:
                assertEquals((int) ((END_TIME - timestamp) + 1),
              ...
              case 2:
                ...
                break;
              case 3:
                assertEquals((float) (timestamp + 3 + 0.01), changeResultToFloat(result.get(j)),
                    (float) delta);
                break;
              case 4:
                assertEquals((4 + (END_TIME - timestamp) + START_TIME + 0.01) * 999,
      ...
      SessionAggregateQueryDataSet dtDelColAvgDataSet = session
          .aggregateQuery(dTDeleteAggrPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
...
//                    assertEquals("null", new String((byte[]) dtDelColAvgResult[i]));
          ...
          switch (currPathPos) {
            case 1:
              assertEquals((START_TIME + END_TIME) / 2.0 + 1,
                  changeResultToDouble(dtDelColAvgResult[i]), delta);
              break;
            case 2:
            ...
            case 3:
              assertEquals((START_TIME + END_TIME) / 2.0 + 3 + 0.01,
                  changeResultToDouble(dtDelColAvgResult[i]), delta * 10000);
              break;
            case 4:
              assertEquals((START_TIME + END_TIME) * 999 / 2.0 + 4.01 * 999,
                  changeResultToDouble(dtDelColAvgResult[i]), delta * 10000);
    ...
    // Test max function
    SessionAggregateQueryDataSet mulStMaxDataSet = session
        .aggregateQuery(mulStPaths, START_TIME, END_TIME + 1, AggregateType.MAX);
    ...
    // Test avg function
    SessionAggregateQueryDataSet mulStAvgDataSet = session
        .aggregateQuery(mulStPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    for (int i = 0; i < mulStQueryLen; i++) {
      assertEquals(getPathNum(mulStAvgResPaths.get(i)) + (START_TIME + END_TIME) / 2.0,
          changeResultToDouble(mulStAvgResult[i]), delta);
    }
    ...
    for (int i = 0; i < mulTimeQueryLen; i++) {
      mulTimeInsertTasks[i] = new MultiThreadTask(1, mulTimePaths, START_TIME + i,
          END_TIME - (4 - i),
    ...
    //query
    // TODO change the simple query and one of the avg query to multithread
    SessionQueryDataSet mulTimeQueryDataSet = session
        .queryData(mulTimePaths, START_TIME, END_TIME + 1);
    ...
    // Test max function
    SessionAggregateQueryDataSet mulTimeMaxDataSet = session
        .aggregateQuery(mulTimePaths, START_TIME, END_TIME + 1, AggregateType.MAX);
    ...
    // Test avg function
    SessionAggregateQueryDataSet mulTimeAvgDataSet = session
        .aggregateQuery(mulTimePaths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    if (isAbleToDelete) {
      ...
      Thread.sleep(1000);
      SessionQueryDataSet beforePSDataSet = session
          .queryData(mulDelPSPaths, START_TIME, END_TIME + 1);
      ...
      // Test avg function
      SessionAggregateQueryDataSet delPSAvgDataSet = session
          .aggregateQuery(mulDelPSPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      for (int i = 0; i < mulDelPSLen; i++) {
        double avg = ((START_TIME + END_TIME) * TIME_PERIOD / 2.0
            - (delStartTime + delEndTime - 1) * delTimePeriod / 2.0) / (TIME_PERIOD
            - delTimePeriod);
      ...
      Thread.sleep(1000);
      SessionQueryDataSet beforePTDataSet = session
          .queryData(mulDelPTPaths, START_TIME, END_TIME + 1);
      ...
      // Test avg function
      SessionAggregateQueryDataSet delPTAvgDataSet = session
          .aggregateQuery(mulDelPTPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      for (int i = 0; i < mulDelPTLen; i++) {
        double avg = ((START_TIME + END_TIME) * TIME_PERIOD / 2.0
            - (delPTStartTime + delPTEndTime) * delPTTimePeriod / 2.0) / (TIME_PERIOD
            - delPTTimePeriod);
      ...
      // Test avg function
      SessionAggregateQueryDataSet delASAvgDataSet = session
          .aggregateQuery(mulDelASPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
...
//                    assertEquals("null", new String((byte[]) delASAvgResult[i]));
      ...
      // Test avg function
      SessionAggregateQueryDataSet delATAvgDataSet = session
          .aggregateQuery(mulDelATPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
...
//                    assertEquals("null", new String((byte[]) delATAvgResult[i]));
    ...
    //query Test
    SessionQueryDataSet addStQueryDataSet = session
        .queryData(addStoragePaths, START_TIME, END_TIME + 1);
    ...
    //aggr Count
    SessionAggregateQueryDataSet addStCountDataSet = session
        .aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.COUNT);
    ...
    //aggr Avg
    SessionAggregateQueryDataSet addStAvgDataSet = session
        .aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    if (isAbleToDelete) {
      ...
      Thread.sleep(1000);
      SessionQueryDataSet stDelPartDataSet = session
          .queryData(addStoragePaths, START_TIME, END_TIME + 1);
      //query
      ...
      // Test avg for the delete
      SessionAggregateQueryDataSet stDelPartAvgDataSet = session
          .aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      // Test count for the delete
      SessionAggregateQueryDataSet stDelPartCountDataSet = session
          .aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.COUNT);
    ...
    public void run() {
      switch (type) {
        ...
        case 1:
          ...
          try {
            localSession
                .insertNonAlignedColumnRecords(path, timestamps, valuesList, dataTypeList, null);
