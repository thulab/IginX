...
public class InfluxDBQueryRowStream implements RowStream {
  ...
  public InfluxDBQueryRowStream(List<FluxTable> tables) {
    this.tables = tables.stream().filter(e -> e.getRecords().size() > 0)
    ...
    for (FluxTable table : this.tables) {
      ...
      if (table.getRecords().get(0).getValueByKey("t") == null) {
        ...
        path = table.getRecords().get(0).getMeasurement() + "." + table.getRecords().get(0)
            .getValueByKey(InfluxDBSchema.TAG) + "." + table.getRecords().get(0).getField();
      }
      DataType dataType = fromInfluxDB(
          table.getColumns().stream().filter(x -> x.getLabel().equals("_value"))
              .collect(Collectors.toList()).get(0).getDataType());
