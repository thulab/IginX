...
public class QueryClientImpl extends AbstractFunctionClient implements QueryClient {
  ...
  private IginXTable simpleQuery(SimpleQuery query) {
    ...
    QueryDataReq req = new QueryDataReq(sessionId,
        MeasurementUtils.mergeAndSortMeasurements(measurements), query.getStartTime(),
        query.getEndTime());
  ...
  private IginXTable aggregateQuery(AggregateQuery query) throws IginXException {
    List<String> measurements = new ArrayList<>(query.getMeasurements());
    AggregateQueryReq req = new AggregateQueryReq(sessionId,
        MeasurementUtils.mergeAndSortMeasurements(measurements), query.getStartTime(),
        query.getEndTime(), query.getAggregateType());
  ...
  private IginXTable downsampleQuery(DownsampleQuery query) throws IginXException {
    List<String> measurements = new ArrayList<>(query.getMeasurements());
    DownsampleQueryReq req = new DownsampleQueryReq(sessionId,
        MeasurementUtils.mergeAndSortMeasurements(measurements), query.getStartTime(),
        query.getEndTime(), query.getAggregateType(), query.getPrecision());
  ...
  private IginXTable lastQuery(LastQuery query) throws IginXException {
    List<String> measurements = new ArrayList<>(query.getMeasurements());
    LastQueryReq req = new LastQueryReq(sessionId,
        MeasurementUtils.mergeAndSortMeasurements(measurements), query.getStartTime());
  ...
  public <M> List<M> query(Query query, Class<M> measurementType) throws IginXException {
    ...
    return table.getRecords().stream().map(e -> resultMapper.toPOJO(e, measurementType))
  ...
  private IginXTable buildIginXTable(QueryDataSet dataSet, List<String> measurements,
    ...
    for (int i = 0; i < dataSet.valuesList.size(); i++) {
      ...
      for (int j = 0; j < dataTypes.size(); j++) {
        if (bitmap.get(j)) {
          values.put(columnIndexMap.get(j),
  ...
  private static class ConsumerMapperControlThread<M> extends Thread {
    ...
    public ConsumerMapperControlThread(IginXTable table, Consumer<M> onNext,
        Class<M> measurementType, ResultMapper resultMapper) {
      this.table = table;
      this.onNext = onNext;
