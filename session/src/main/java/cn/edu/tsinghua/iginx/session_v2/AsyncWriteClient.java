...
public interface AsyncWriteClient extends AutoCloseable {
    ...
    void writePoint(final Point pointm, final String timePrecision);

    void writePoints(final List<Point> points);

    void writePoints(final List<Point> points, final String timePrecision);

    void writeRecord(final Record record);

    void writeRecord(final Record record, final String timePrecision);

    void writeRecords(final List<Record> records);

    void writeRecords(final List<Record> records, final String timePrecision);

    <M> void writeMeasurement(final M measurement);

    <M> void writeMeasurement(final M measurement, final String timePrecision);

    <M> void writeMeasurements(final List<M> measurements);

    <M> void writeMeasurements(final List<M> measurements, final String timePrecision);

    void writeTable(final Table table);

    void writeTable(final Table table, final String timePrecision);
