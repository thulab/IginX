...
public class SessionExecuteSqlResult {
  ...
  private void constructQueryResult(ExecuteSqlResp resp) {
    ...
    if (resp.getQueryDataSet() != null) {
      this.values = parseValues(resp.dataTypeList, resp.queryDataSet.valuesList,
  ...
  private List<List<Object>> parseValues(List<DataType> dataTypeList, List<ByteBuffer> valuesList,
    ...
    if (values != null && !values.isEmpty()) {
      ...
      for (int i = 0; i < paths.size(); i++) {
        cache.add(new ArrayList<>(Arrays.asList(
            needFormatTime ? formatTime(timestamps[i], timePrecision)
                : String.valueOf(timestamps[i]),
            paths.get(i),
