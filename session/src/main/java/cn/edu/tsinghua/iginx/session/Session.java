...
public class Session {
  ...
  public void insertColumnRecords(List<String> paths, long[] timestamps, Object[] valuesList,
      ...
      throws SessionException, ExecutionException {
    if (paths.isEmpty() || timestamps.length == 0 || valuesList.length == 0 || dataTypeList
        .isEmpty()) {
    ...
    if (attributesList != null && paths.size() != attributesList.size()) {
      logger.error(
          "The sizes of paths, valuesList, dataTypeList and attributesList should be equal.");
      return;
    }
    ...
    Arrays
        .sort(index, Comparator.comparingLong(Arrays.asList(ArrayUtils.toObject(timestamps))::get));
  ...
  public void insertNonAlignedColumnRecords(List<String> paths, long[] timestamps,
      Object[] valuesList,
      List<DataType> dataTypeList, List<Map<String, String>> attributesList)
      throws SessionException, ExecutionException {
    if (paths.isEmpty() || timestamps.length == 0 || valuesList.length == 0 || dataTypeList
  ...
  public void deleteDataInColumns(List<String> paths, long startTime, long endTime)
      throws SessionException, ExecutionException {
    DeleteDataInColumnsReq req = new DeleteDataInColumnsReq(sessionId, mergeAndSortPaths(paths),
        startTime, endTime);
  ...
  public SessionAggregateQueryDataSet aggregateQuery(List<String> paths, long startTime,
      long endTime, AggregateType aggregateType)
      throws SessionException, ExecutionException {
    AggregateQueryReq req = new AggregateQueryReq(sessionId, mergeAndSortPaths(paths), startTime,
        endTime, aggregateType);
  ...
  public SessionQueryDataSet downsampleQuery(List<String> paths, long startTime, long endTime,
      AggregateType aggregateType, long precision) throws SessionException, ExecutionException {
    DownsampleQueryReq req = new DownsampleQueryReq(sessionId, mergeAndSortPaths(paths), startTime,
        endTime,
  ...
  private List<String> mergeAndSortPaths(List<String> paths) {
    ...
    List<String> prefixes = paths.stream().filter(x -> x.contains("*"))
        .map(x -> x.substring(0, x.indexOf("*"))).collect(Collectors.toList());
    if (prefixes.isEmpty()) {
      Collections.sort(paths);
      return paths;
    }
