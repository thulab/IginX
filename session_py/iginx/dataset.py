...
class AggregateQueryDataSet(object):
  ...
  def __str__(self):
    ...
    if self.__timestamps:
      for i in range(len(self.__timestamps)):
        value += "Time\t" + AggregateType._VALUES_TO_NAMES[self.__type] + "(" + \
                 self.__paths[i] + ")\n"
        value += str(self.__timestamps[i]) + "\t" + str(self.__values[i]) + "\n"
    else:
      for path in self.__paths:
        value += AggregateType._VALUES_TO_NAMES[
                   self.__type] + "(" + path + ")\t"
      value += "\n"
...
class SqlExecuteResult(object):
  ...
  def __init__(self, resp):
    ...
    elif self.__type in [SqlType.AggregateQuery, SqlType.SimpleQuery,
                         SqlType.DownsampleQuery, SqlType.ValueFilterQuery]:
  ...
  def is_query(self):
    return self.__type in [SqlType.AggregateQuery, SqlType.SimpleQuery,
                           SqlType.DownsampleQuery, SqlType.ValueFilterQuery]
