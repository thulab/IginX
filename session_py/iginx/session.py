...
from .utils.byte_utils import timestamps_to_bytes, row_values_to_bytes, \
  column_values_to_bytes, bitmap_to_bytes
...
class Session(object):
  ...
  def add_storage_engine(self, ip, port, type, extra_params=None):
    self.batch_add_storage_engine(
        [StorageEngine(ip, port, type, extraParams=extra_params)])

  def batch_add_storage_engine(self, storage_engines):
    req = AddStorageEnginesReq(sessionId=self.__session_id,
                               storageEngines=storage_engines)
  ...
  def insert_row_records(self, paths, timestamps, values_list, data_type_list):
    if len(paths) == 0 or len(timestamps) == 0 or len(values_list) == 0 or len(
        data_type_list) == 0:
      raise RuntimeError("invalid insert request")
    if len(paths) != len(data_type_list):
      raise RuntimeError(
          "the sizes of paths and data_type_list should be equal")
    if len(timestamps) != len(values_list):
      raise RuntimeError(
          "the sizes of timestamps and values_list should be equal")
    ...
    for i in range(len(timestamps)):
      ...
      values_buffer_list.append(
          row_values_to_bytes(values, sorted_data_type_list))
    ...
    req = InsertRowRecordsReq(sessionId=self.__session_id, paths=paths,
                              timestamps=timestamps_to_bytes(timestamps),
                              valuesList=values_buffer_list,
                              bitmapList=bitmap_buffer_list,
                              dataTypeList=sorted_data_type_list)
    status = self.__client.insertRowRecords(req)
    Session.verify_status(status)

  def insert_non_aligned_row_records(self, paths, timestamps, values_list,
      data_type_list):
    if len(paths) == 0 or len(timestamps) == 0 or len(values_list) == 0 or len(
        data_type_list) == 0:
      raise RuntimeError("invalid insert request")
    ...
    req = InsertNonAlignedRowRecordsReq(sessionId=self.__session_id,
                                        paths=paths,
                                        timestamps=timestamps_to_bytes(
                                            timestamps),
                                        valuesList=values_buffer_list,
                                        bitmapList=bitmap_buffer_list,
                                        dataTypeList=sorted_data_type_list)
  ...
  def insert_column_records(self, paths, timestamps, values_list,
      data_type_list):
    if len(paths) == 0 or len(timestamps) == 0 or len(values_list) == 0 or len(
        data_type_list) == 0:
      raise RuntimeError("invalid insert request")
    ...
    for i in range(len(paths)):
      values = sorted_values_list[i]
      values_buffer_list.append(
          column_values_to_bytes(values, sorted_data_type_list[i]))
  ...
  def insert_non_aligned_column_records(self, paths, timestamps, values_list,
      data_type_list):
    if len(paths) == 0 or len(timestamps) == 0 or len(values_list) == 0 or len(
        data_type_list) == 0:
      raise RuntimeError("invalid insert request")
    ...
    req = InsertNonAlignedColumnRecordsReq(sessionId=self.__session_id,
                                           paths=paths,
                                           timestamps=timestamps_to_bytes(
                                               timestamps),
                                           valuesList=values_buffer_list,
  ...
  def query(self, paths, start_time, end_time):
    req = QueryDataReq(sessionId=self.__session_id,
                       paths=Session.merge_and_sort_paths(paths),
    ...
    return QueryDataSet(paths, data_types, raw_data_set.timestamps,
                        raw_data_set.valuesList, raw_data_set.bitmapList)

  def last_query(self, paths, start_time=0):
    if len(paths) == 0:
      logger.warning("paths shouldn't be empty")
      return None
    req = LastQueryReq(sessionId=self.__session_id,
                       paths=Session.merge_and_sort_paths(paths),
  ...
  def aggregate_query(self, paths, start_time, end_time, type):
    req = AggregateQueryReq(sessionId=self.__session_id, paths=paths,
                            startTime=start_time, endTime=end_time,
                            aggregateType=type)
  ...
  def batch_delete_data(self, paths, start_time, end_time):
    req = DeleteDataInColumnsReq(sessionId=self.__session_id, paths=paths,
                                 startTime=start_time, endTime=end_time)
    status = self.__client.deleteDataInColumns(req)
    Session.verify_status(status)

  def add_user(self, username, password, auths):
    req = AddUserReq(sessionId=self.__session_id, username=username,
                     password=password, auths=auths)
