...
public class OpenTSDBStorage implements IStorage {
  ...
  private TaskExecuteResult executeInsertTask(String storageUnit, Insert insert) {
    ...
    if (e != null) {
      return new TaskExecuteResult(null,
          new PhysicalException("execute insert task in opentsdb failure", e));
    }
    return new TaskExecuteResult(null, null);
  }
    ...
    for (int i = 0; i < data.getTimeSize(); i++) {
      ...
      for (int j = 0; j < data.getPathNum(); j++) {
        if (bitmapView.get(j)) {
          ...
          switch (type) {
            case BOOLEAN:
              points.add(Point.metric(schema.getMetric()).tag(schema.getTags())
                  .tag(DATA_TYPE, type.toString())
                  .value(data.getTimestamp(i), (boolean) data.getValue(i, index) ? 1 : 0).build());
              break;
            case INTEGER:
              points.add(Point.metric(schema.getMetric()).tag(schema.getTags())
                  .tag(DATA_TYPE, type.toString())
                  .value(data.getTimestamp(i), (int) data.getValue(i, index)).build());
              break;
            case LONG:
              points.add(Point.metric(schema.getMetric()).tag(schema.getTags())
                  .tag(DATA_TYPE, type.toString())
                  .value(data.getTimestamp(i), (long) data.getValue(i, index)).build());
              break;
            case FLOAT:
              points.add(Point.metric(schema.getMetric()).tag(schema.getTags())
                  .tag(DATA_TYPE, type.toString())
                  .value(data.getTimestamp(i), (float) data.getValue(i, index)).build());
              break;
            case DOUBLE:
              points.add(Point.metric(schema.getMetric()).tag(schema.getTags())
                  .tag(DATA_TYPE, type.toString())
                  .value(data.getTimestamp(i), (double) data.getValue(i, index)).build());
              break;
            case BINARY:
  ...
  private TaskExecuteResult executeProjectTask(TimeInterval timeInterval, String storageUnit,
      Project project) {
    Query.Builder builder = Query.begin(timeInterval.getStartTime()).end(timeInterval.getEndTime())
        .msResolution();
